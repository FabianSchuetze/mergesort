...
Storage::Storage(const std::vector<int>& data)
    : _data(data),
...
void Storage::initialize_gpu_memory() {
    unsigned int nBytes = _data.size()* sizeof(int);
...
const std::vector<int>& Storage::return_data_const() {
    sync_to_cpu();
    return _data;
}

std::vector<int>& Storage::return_data() {
    sync_to_cpu();
    recent_head = "CPU";
    return _data;
}
